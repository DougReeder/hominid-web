...
Unfortunately, the *filesystem* isn't necessarily re-sized, so it might make sense to set a larger capacity from the start.
...
If PostgreQL can be configured to not care about the `lost+found` directory, the `pgsql-pvc` `PersistentVolumeClaim` could also use the default `StorageClass`.
